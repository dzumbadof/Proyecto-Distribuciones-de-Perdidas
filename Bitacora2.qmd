...
Se cargan los datos al estilo David, no entiendo cual es el problema con hacerlo asi Oye, tranquilo, viejo
...
En la figura \ref{fig:CantReportAño} se observa que en el periodo considerado la cantidad de reclamos a  TSA fue máxima en 2004 y, desde entonces, experimenta un comportamiento decreciente.


```{r, fig.lp="CantReportAño", fig.cap="Cantidad de reportes por año}
# a <- min(year(date_received), na.rm = T)

# datos %>% count(year(date_received))

g <- ggplot(datos, aes(year(date_received)))

g <- g + stat_count(geom='bar', aes(y=..count..), color="green") 

g <- g +  scale_x_continuous(breaks = c(2002:2015), limits = c(2001, 2016))

g <- g + labs( x = "Años",
   y = "Cantidad de reportes",
   caption = "Fuente: Elaboración propia con datos de TSA")

g <- g + scale_fill_manual(values = c("blue"))

g <- g + theme_minimal_hgrid()
...
2). Asimetría de los montos pagados 
